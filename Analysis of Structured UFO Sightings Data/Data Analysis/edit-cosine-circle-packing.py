...
def createCluster(inputCSV,argNum):
    ...
    for i in range(len(row)):
        if "x-coordinate" in row[i][0].split(","):
            ...
            data[str(int(float(column[argNum])*100/25))]=[]         #Cluster based on the argument number passed
    ...
    for i in range(len(row)):
        if "x-coordinate" in row[i][0].split(","):
            ...
            data[str(int(float(column[argNum])*100/25))].append(second)          #Cluster based on the argument number passed
